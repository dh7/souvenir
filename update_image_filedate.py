...
def change_file_date_to_exif_date(file_path, default_date=None):
    try:
...
def process_folder(folder_path, default_date=None):
    for file_path in glob.glob(os.path.join(folder_path, "*.jpg")) + glob.glob(
        os.path.join(folder_path, "*.jpeg")
    ):
        change_file_date_to_exif_date(file_path, default_date)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Update image file dates based on EXIF data or a default date."
    )
    ...
    process_folder(args.folder, args.default_date)
